...
def gptmodel_forward_qwen2_5_vl(
    ...
    **kwargs,
    ...
    if pack_seqs:
        ...
        if "pixel_values" in multi_modal_inputs.keys():
            output_orig = model(
                input_ids=input_ids_rmpad,
                attention_mask=None,
                position_ids=position_ids,
        ...
        if "pixel_values_videos" in multi_modal_inputs.keys():
            output_orig = model(
                input_ids=input_ids_rmpad,
                attention_mask=None,
                position_ids=position_ids,
                packed_seq_params=packed_seq_params,
                pixel_values_videos=multi_modal_inputs["pixel_values_videos"].to(input_ids.device),
                video_grid_thw=multi_modal_inputs["video_grid_thw"].to(input_ids.device),
            )
        
        if "pixel_values_videos" not in multi_modal_inputs.keys() and "pixel_values" not in multi_modal_inputs.keys():
            output_orig = model(
                input_ids=input_ids_rmpad,
                attention_mask=None,
                position_ids=position_ids,
